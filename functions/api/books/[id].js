...
export async function onRequestGet({ request, env, params }) {
  ...
  try {
    const bookRow = await env.DB.prepare('SELECT id, title, uploader, visibility, created_at FROM books WHERE id = ?').bind(id).first();
    ...
    return json(request, { ok: true, book: {
      id: Number(bookRow.id), title: bookRow.title, uploader: bookRow.uploader, created_at: Number(bookRow.created_at)
